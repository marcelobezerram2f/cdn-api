...
use App\Services\Acme\Account;
use App\Services\Acme\Authorization;
use App\Services\Acme\Certificate;
use App\Services\Acme\Challenge;
use App\Services\Acme\Order;
...
class Client
{
     ...
     * Live url
     */
    const DIRECTORY_LIVE = 'https://acme.zerossl.com/v2/DV90';

    /**
     * Staging url
     */
    const DIRECTORY_STAGING = 'https://acme.zerossl.com/v2/DV90';
     ...
     * Revoke Certificate
     */
    const DIRECTORY_REVOKE_CERT = 'revokeCert';
     ...
     * @throws \Exception
     */
    public function createOrder(array $domains): array|Order
    {
        $identifiers = [];
        ...
        if ($response->getStatusCode() >= 400) {
            return [
                ...
                "message" => isset($data['detail']) ? $data['detail'] : "Unknown error, contact support",
                "errors" => "Certificate generation order request failed"
            ];
        }
        ...
        foreach ($order->getAuthorizationURLs() as $authorizationURL) {
            ...
            if ($response->getStatusCode() >= 400) {
                return [
                    "code" => $response->getStatusCode(),
                    "message" => isset($data['detail']) ? $data['detail'] : "Unknown error, contact support",
                    "errors" => "Certificate generation order authorization failed"
                ];
            }
     ...
     * @throws \Exception
     */
    public function getCertificate(Order $order): array|Certificate
    {
        ...
        if ($response->getStatusCode() >= 400) {
            ...
            if ($data['status'] != "valid") {
                /* LetsencryptCertificateReprocessing::create([
                     'status' => $data['status'],
                     'domain' => $data['identifiers'][0]['value'],
                     'csr' => $csr,
                     'private_key'=>$privateKey,
                     'url' => $order->getFinalizeURL(),
                     'payload' => json_encode($payload)
                 ]);*/

                return ['not_valid' => true, 'status' => $data['status'], 'errors' => 'completion of the SSL certificate generation order did not validate the request.'];
            }
     ...
     */
    protected function request($url, $payload = [], $method = 'POST'): array|ResponseInterface
    {
        $logEvent = new EventLogService();
        try {
            Log::info("Requisição tipo $method.  na  URL $url");
            ...
            Log::info("retorno de REQUEST . " . json_encode(json_decode($response->getBody(), true)) . " - HTTP CODE: " . $response->getStatusCode());


            $body = json_decode($response->getBody(), true);
            if ($response->getStatusCode() == 200) {
                $logEvent->syslog(
                    '[CDN-API | ACME-LETSENCRYPT] Requisição ao endpoint' . $url . ', retornou com sucesso',
                    "Retorno da requisição : " . isset($body['status']) ? $body['status'] : "status not found" . "HTTP CODE : " . $response->getStatusCode(),
                    'error',
                    $this->facilityLog . ':' . basename(__FUNCTION__)
                );
            } else if ($response->getStatusCode() >= 400) {
                $logEvent->syslog(
                    '[CDN-API | ACME-LETSENCRYPT] Requisição ao endpoint' . $url . ', retornou com falha',
                    "Retorno da requisição : " . json_encode($body) . "HTTP CODE : " . $response->getStatusCode(),
                    'error',
                    $this->facilityLog . ':' . basename(__FUNCTION__)
                );
            } else {
                $logEvent->syslog(
                    '[CDN-API | ACME-LETSENCRYPT] Requisição ao endpoint' . $url . ', retornou  status code não identificado',
                    "Retorno da requisição : " . json_encode($body) . "HTTP CODE : " . $response->getStatusCode(),
                    'error',
                    $this->facilityLog . ':' . basename(__FUNCTION__)
                );
    ...
    public function revokeCertificate($certificate): array
    {
        $url = $this->getUrl(self::DIRECTORY_REVOKE_CERT);
        $certDer = $this->convertPemToDer($certificate);
        $certBase64 = base64_encode($certDer);

        // Criar a carga útil assinada
        $payload = $this->signPayloadJWK(
            [
                'certificate' => $certDer,
                'reason' => 0
            ],
        ...
        if ($response->getStatusCode() >= 400) {
            return [
                "code" => $response->getStatusCode(),
                "message" => isset($data['detail']) ? $data['detail'] : "Unknown error, contact support",
                "errors" => "Certificate revoke failed"
            ];
        }
    ...
    private function convertPemToDer(string $certificate): string
    {
